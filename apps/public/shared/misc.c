...
{	
	...
	snprintf(buf, sizeof(buf), "DeviceInfo_PVC%d", wan_primary_ifunit());
	tcapi_get(buf, "WanIP", ip);
	return ip;
...
int get_wanstate(void)
{
	...
	snprintf(buf, sizeof(buf), "wan%d_state_t", wan_primary_ifunit());

	return tcapi_get_int("Wanduck_Common", buf);
}
...
#if 0
static u_int32_t crc_table[256]={ 0x00000000,0x77073096,0xEE0E612C,0x990951BA,
...
u_int32_t crc_calc(uint32 crc, char *buf, int len)
{
	while (len-- > 0) {
...
u_int32_t netdev_calc(char *ifname, char *ifname_desc, unsigned long *rx, unsigned long *tx, char *ifname_desc2, unsigned long *rx2, unsigned long *tx2)
{
...
{
	static char productid[16] = {0};
	tcapi_get("SysInfo_Entry", "ProductName", productid);
	return productid;
}
