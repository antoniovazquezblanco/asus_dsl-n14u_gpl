...
{
	char rcsupport[MAXLEN_TCAPI_MSG] = {0};
	char features[MAXLEN_TCAPI_MSG] = {0};

	if (!(feature && *feature))
		return;

	if(tcapi_get("SysInfo_Entry", "rc_support", rcsupport) || strlen(rcsupport) == 0)
		tcapi_set("SysInfo_Entry", "rc_support", feature);
	else {
		sprintf(features, "%s %s", rcsupport, feature);
		tcapi_set("SysInfo_Entry", "rc_support", features);
	}
}
...
/*	char tmp[100], prefix[32]="wanXXXXXX_";
	...
	for(unit = WAN_UNIT_FIRST; unit < WAN_UNIT_MAX; ++unit){
		...
		if(!strncmp(ifname, "ppp", 3)){
			...
			tcapi_get("", strcat_r(prefix, "pppoe_ifname", tmp), value);
			if(!strcmp(value, ifname))
				return unit;
		}
		else {
			// if(nvram_match(strcat_r(prefix, "ifname", tmp), ifname))
			tcapi_get("", strcat_r(prefix, "ifname", tmp), value);
			if(!strcmp(value, ifname))
				return unit;
		}
	}
...
{
	char node[16] = {0};
	static char ifname[8] = {0};

#ifdef TCSUPPORT_MULTISERVICE_ON_WAN
	if(unit >= 8)
	{
		sprintf(node, "WanExt_PVC%de0", unit);
		if(tcapi_match(node, "ISP", "2"))
			sprintf(ifname, "ppp%d0", unit);
		else
			sprintf(ifname, "nas%d_0", unit);
	}
	else
#endif
	{
		...
		if(tcapi_match(node, "ISP", "2"))
			sprintf(ifname, "ppp%d", unit);
		else
			sprintf(ifname, "nas%d", unit);
	}
	return ifname;
}
...
#ifdef TCSUPPORT_MULTISERVICE_ON_WAN
		if(unit >= 8)
			sprintf(prefix, "WanExt_PVC%de0", unit);
		else
#endif
		sprintf(prefix, "Wan_PVC%d", unit);
		if(tcapi_match(prefix, "Active", "Yes"))
			if(tcapi_match(prefix, "DEFAULTROUTE", "Yes"))
				return unit;
	}
...
char *get_usb_ehci_port(int port)
{
	...
	tcapi_get("USB_Entry", "ehci_ports", ehci_ports);

	//foreach(word, nvram_safe_get("ehci_ports"), next) {
	foreach(word, ehci_ports, next) {
		if(i==port) {
...
char *get_usb_ohci_port(int port)
{
	...
	tcapi_get("USB_Entry", "ohci_ports", ohci_ports);

	//foreach(word, nvram_safe_get("ohci_ports"), next) {
	foreach(word, ohci_ports, next) {
		if(i==port) {
...
int get_usb_port_number(const char *usb_port){
	...
	port_num = 0;
	i = 0;
	tcapi_get("USB_Entry", "ehci_ports", xhci_ports);
	//foreach(word, nvram_safe_get("ehci_ports"), next){
	foreach(word, xhci_ports, next){
		++i;
	...
	i = 0;
	if(port_num == 0){
		tcapi_get("USB_Entry", "ohci_ports", xhci_ports);
		//foreach(word, nvram_safe_get("ohci_ports"), next){
		foreach(word, xhci_ports, next){
			++i;
			if(!strcmp(usb_port, word)){
