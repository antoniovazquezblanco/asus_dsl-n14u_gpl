...
int set_usb_common_nvram(const char *action, const char *usb_node, const char *known_type){
	...
	if(!check_hotplug_action(action)){
		tcapi_set(USB_DATA, prefix, "");
		tcapi_set(USB_DATA, strcat_r(prefix, "_removed", nvram_name), "0");
		tcapi_set(USB_DATA, strcat_r(prefix, "_vid", nvram_name), "");
		tcapi_set(USB_DATA, strcat_r(prefix, "_pid", nvram_name), "");
		tcapi_set(USB_DATA, strcat_r(prefix, "_manufacturer", nvram_name), "");
		tcapi_set(USB_DATA, strcat_r(prefix, "_product", nvram_name), "");
		tcapi_set(USB_DATA, strcat_r(prefix, "_serial", nvram_name), "");

		partition_order = 0;
		memset(nvram_name, 0, 32);
		sprintf(nvram_name, "%s_fs_path%d", prefix, partition_order);
		tcapi_get(USB_DATA, nvram_name, tcapi_retval);
		if(strlen(tcapi_retval) > 0){
			tcapi_set(USB_DATA, nvram_name, "");
		...
		tcapi_get(USB_DATA, prefix, tcapi_retval);
		// strcpy(been_type, nvram_safe_get(prefix));
		strcpy(been_type, tcapi_retval);
...
#if defined(TCSUPPORT_USB_PRINTER_SERVER) || defined(TCSUPPORT_USB_3G_DONGLE)
				if(
// #ifdef RTCONFIG_USB_PRINTER
#ifdef TCSUPPORT_USB_PRINTER_SERVER
...
// #ifdef RTCONFIG_USB_MODEM
		...
		if(known_type != NULL)
			...
			tcapi_set(USB_DATA, prefix, known_type);
		else if(get_device_type_by_node(usb_node, type, 16) != NULL)
			// nvram_set(prefix, type);
			tcapi_set(USB_DATA, prefix, type);
		else // unknown device.
			return 0;

		strcat_r(prefix, "_vid", nvram_name);
		if(get_usb_vid(usb_node, vid, 8) == NULL)
			// nvram_set(nvram_name, "");
			tcapi_set(USB_DATA, nvram_name, "");
		else
			// nvram_set(nvram_name, vid);
			tcapi_set(USB_DATA, nvram_name, vid);

		strcat_r(prefix, "_pid", nvram_name);
		if(get_usb_pid(usb_node, pid, 8) == NULL)
			// nvram_set(nvram_name, "");
			tcapi_set(USB_DATA, nvram_name, "");
		else
			// nvram_set(nvram_name, pid);
			tcapi_set(USB_DATA, nvram_name, pid);

		strcat_r(prefix, "_manufacturer", nvram_name);
		if(get_usb_manufacturer(usb_node, manufacturer, 256) == NULL)
			// nvram_set(nvram_name, "");
			tcapi_set(USB_DATA, nvram_name, "");
		else
			// nvram_set(nvram_name, manufacturer);
			tcapi_set(USB_DATA, nvram_name, manufacturer);

		strcat_r(prefix, "_product", nvram_name);
		if(get_usb_product(usb_node, product, 256) == NULL)
			// nvram_set(nvram_name, "");
			tcapi_set(USB_DATA, nvram_name, "");
		else
			// nvram_set(nvram_name, product);
			tcapi_set(USB_DATA, nvram_name, product);

		strcat_r(prefix, "_serial", nvram_name);
		if(get_usb_serial(usb_node, serial, 256) == NULL)
			// nvram_set(nvram_name, "");
			tcapi_set(USB_DATA, nvram_name, "");
		else
			// nvram_set(nvram_name, serial);
			tcapi_set(USB_DATA, nvram_name, serial);
	}
...
// #ifdef RTCONFIG_USB
	...
	char tcapi_retval[16], tcapi_retval2[16];
	usb_dbg("(%s): action=%s.\n", device_name, action);

	tcapi_get(USB_DATA, "stop_sd", tcapi_retval);
	// if(!strcmp(nvram_safe_get("stop_sd"), "1")){
	if(!strcmp(tcapi_retval, "1")){
	...
	if(!check_hotplug_action(action)){
		...
		tcapi_get(USB_DATA, "usb_path1_act", tcapi_retval);
		tcapi_get(USB_DATA, "usb_path2_act", tcapi_retval2);
		// if(!strcmp(nvram_safe_get("usb_path1_act"), device_name)){
		if(!strcmp(tcapi_retval, device_name)){
			strcpy(usb_port, USB_EHCI_PORT_1);

			// nvram_set("usb_path1_act", "");
			tcapi_set(USB_DATA, "usb_path1_act", "");
		}
		// else if(!strcmp(nvram_safe_get("usb_path2_act"), device_name)){
		else if(!strcmp(tcapi_retval2, device_name)){
			strcpy(usb_port, USB_EHCI_PORT_2);

			// nvram_set("usb_path2_act", "");
			tcapi_set(USB_DATA, "usb_path2_act", "");
		}
...
// #ifdef RTCONFIG_USB_PRINTER
	...
	strcpy(nvram_value, tcapi_retval);
...
// 201102. James. Move the Jiahao's code from mdev. {
	if(strlen(device_name) == 3){ // sda, sdb, sdc...
		...
		tcapi_get(USB_DATA, nvram_name, tcapi_retval);
		if(strlen(tcapi_retval) <= 0)
			tcapi_set(USB_DATA, nvram_name, device_name);
		// if(strlen(nvram_safe_get(nvram_name)) <= 0)
		...
		if(!strcmp(tcapi_retval, "storage")){
			memset(nvram_name, 0, 32);
...
// #ifdef RTCONFIG_USB_PRINTER
	...
	tcapi_get(USB_DATA, "stop_lp", tcapi_retval);
	if(!strcmp(tcapi_retval, "1")){
	...
	tcapi_get(USB_DATA, "usb_path2", tcapi_retval);
	tcapi_get(USB_DATA, "usb_path1", tcapi_retval2);
	// if((port_num == 1 && !strcmp(nvram_safe_get("usb_path2"), "printer"))
			// || (port_num == 2 && !strcmp(nvram_safe_get("usb_path1"), "printer"))
	if((port_num == 1 && !strcmp(tcapi_retval, "printer"))
			|| (port_num == 2 && !strcmp(tcapi_retval2, "printer"))
			){
...
// #ifdef RTCONFIG_USB_MODEM
	...
	tcapi_get(USB_DATA, "stop_sg", tcapi_retval);
	...
	if(!strcmp(vid, "1199"))
		...
		tcapi_get(USB_DATA, "stop_sg_remove", tcapi_retval);
...
// #ifdef RTCONFIG_USB_MODEM
	...
	tcapi_get(USB_DATA, "stop_cd", tcapi_retval);
	...
	tcapi_get(USB_DATA, "stop_cd_remove", tcapi_retval);
	if(strcmp(tcapi_retval, "1")){
...
// #ifdef RTCONFIG_USB_MODEM
	...
	tcapi_get(USB_DATA, "stop_modem", tcapi_retval);
	...
	if(!check_hotplug_action(action)){
		...
		if(!strcmp(tcapi_retval, device_name)){
			...
			tcapi_set(USB_DATA, "usb_path1_act", "");
			tcapi_set(USB_DATA, "usb_path1_act_def", "");
		}
		// else if(!strcmp(nvram_safe_get("usb_path2_act"), device_name)){
		else if(!strcmp(tcapi_retval2, device_name)){
			strcpy(usb_port, USB_EHCI_PORT_2);
			...
			tcapi_set(USB_DATA, "usb_path2_act", "");
			tcapi_set(USB_DATA, "usb_path2_act_def", "");
		}
...
usb_dbg("%s: kill pppd(%d).\n", __FUNCTION__, pppd_pid);
	...
	if((port_num == 1 && !strcmp(tcapi_retval, "modem"))
			|| (port_num == 2 && !strcmp(tcapi_retval2, "modem"))
			){
	...
	strcpy(current_value, tcapi_retval);
	...
	strcpy(current_def, tcapi_retval);
	...
	if(isSerialNode(device_name)){
		...
		if(!got_Int_endpoint){
			...
			if(!strcmp(device_name, "ttyUSB0") && !strcmp(current_value, "")){
				...
				tcapi_set(USB_DATA, nvram_def, "1");
				memset(current_def, 0, 16);
...
usb_dbg("(%s): got tty nodes and notify restart wan(%d)...\n", device_name, wan_unit);
			...
			// notify_rc_and_wait(cmd);	//tmp disable
			...
			tcapi_get(USB_DATA, nvram_name, tcapi_retval);
			tcapi_get(USB_DATA, nvram_name2, tcapi_retval2);
			// init_3g_param(nvram_safe_get(nvram_name), nvram_safe_get(nvram_name2), port_num);
			init_3g_param(tcapi_retval, tcapi_retval2, port_num);
#ifdef RTCONFIG_DUALWAN
		}
		else
...
int asus_usbbcm(const char *device_name, const char *action){
	...
	int isLock;
	char nvram_name[32];
	char tcapi_retval[16], tcai_retval2[8];
	usb_dbg("(%s): action=%s.\n", device_name, action);
...
#if defined(TCSUPPORT_USBHOST) || defined(TCSUPPORT_USB_PRINTER_SERVER) || defined(TCSUPPORT_USB_3G_DONGLE)
	char usb_port[8], usb_node[8];
	int port_num;
// #ifdef RTCONFIG_USB_MODEM
#ifdef TCSUPPORT_USB_3G_DONGLE
	...
	tcapi_get(USB_DATA, "stop_ui", tcapi_retval);
	...
	if(!check_hotplug_action(action)){
		...
		tcapi_get(USB_DATA, nvram_usb_path, tcapi_retval);
// #ifdef RTCONFIG_USB_PRINTER
#ifdef TCSUPPORT_USB_PRINTER_SERVER
		// if(!strcmp(nvram_safe_get(nvram_usb_path), "printer"))
		if(!strcmp(tcapi_retval, "printer"))
...
// #ifdef RTCONFIG_USB_MODEM
	...
	if(!strcmp(vid, "1199") && isStorageInterface(device_name)){
		if(init_3g_param(vid, pid, port_num)){
			...
			tcapi_get(USB_DATA, "stop_ui_remove", tcapi_retval);
...
// #ifdef RTCONFIG_USB_MODEM
	...
	if(is_beceem_dongle(1, vid, pid)){
		...
		tcapi_get(USB_DATA, "modem_isp", tcapi_retval);
		// char *isp = nvram_safe_get("modem_isp");
		char *isp = tcapi_retval;
