...
#define CONFIG_USBINFO_PATH "/opt/etc/webdav_usbinfo.conf"  //save the url-disk-id
#define USBINFO_PATH "/tmp/webdav_client_usbinfo"    //save the usbinfo now
...
#define CONFIG_USBINFO_PATH "/tmp/webdav_usbinfo.conf"  //save the url-disk-id
#define USBINFO_PATH "/tmp/webdav_client_usbinfo"    //save the usbinfo now
#endif
#else
#define CONFIG_PATH "/tmp/smartsync/webdav/config/webdav.conf"
...
typedef struct SYNC_LIST{
    ...
    Server_TreeNode *RealOldServerRootNode;
    int index;
}sync_list;
sync_list **g_pSyncList;

struct pre_disk_info_tag
{
    char serial[50];
    char product[10];
    char vonder[10];
...
struct disk_info *config_disk_info,*config_disk_info_start,*config_disk_tmp;
